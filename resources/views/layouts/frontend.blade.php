...
<body>
    <nav class="navbar navbar-expand-md fixed-top navbar-dark bg-dark text-white mb-5" style="background-color: #171d26!important;">
        <a class="navbar-brand" href="{{ url('/') }}">
            <img src="{{ asset('images/logo-mamen.png') }}" width="50" class="d-inline-block align-top" alt="" loading="lazy">
        </a>
        ...
        <ul class="nav navbar-nav navbar-right ml-auto">
            @if (Auth::guard()->check())
                <ul class="navbar-nav d-none d-md-block ml-4">
                    ...
                    <a class="nav-link dropdown-toggle font-weight-bold text-white" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        <i class="fa fa-user-circle"></i> {{ Auth::user()->nama }}
                    </a>
                    <div class="dropdown-menu border-0 shadow" aria-labelledby="navbarDropdown">
                        <a class="dropdown-item" href="{{ route('console.dashboard.index') }}">
            ...
            @else
                <li class="nav-item">
                    <a href="{{ route('auth.login') }}" class="btn btn-md shadow btn-primary btn-block">
